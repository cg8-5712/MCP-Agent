...
### Phase 3: 向量检索与 LLM 集成

**目标**：实现智能工具检索与任务规划

- [ ] 工具描述向量化（集成 Embedding 模型）
- [ ] 向量数据库集成（FAISS/pgvector/Milvus）
- [ ] Tool Embedding Search API
- [ ] Planner LLM 集成（生成执行计划）
- [ ] Executor 实现（多步骤任务执行）
- [ ] Final LLM 集成（自然语言回答生成）
- [ ] 任务链路追踪与错误回溯

**预期交付**：端到端的 AI Agent 调用链路
...
- Go 1.21+
- PostgreSQL 或 SQLite
- （可选）向量数据库：FAISS / pgvector / Milvus
